...
import { getAllWishlists, createWishlist } from '../../services/wishlist.service'
...
const WishlistCard: React.FC<WishlistCardProps> = ({ wishlist, onEdit, onDelete, alwaysShowActions, showUsersInterested = true }) => {
    const { user } = useAuth()
    const isOwner = user && String(user.user_id) === String(wishlist.user_id)
    const [showInterested, setShowInterested] = useState(false)
    const [interestedWishlists, setInterestedWishlists] = useState<WishlistResponseDto[]>([])
    ...
    const handleConfirmAdd = async () => {
        ...
        try {
            await createWishlist({
                ...
                name: wishlist.name,
                region: wishlist.region,
                theme: wishlist.theme,
                is_public: true
            })
            setShowConfirmModal(false)
        } catch {
            setAddError('Failed to add to wishlist')
        } finally {
            setAddLoading(false)
        }
    }

    const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}${PATHS.WISHLIST_SHARE.replace(':id', wishlist.wishlist_id)}` : ''
    ...
    return (
        <div className="bg-white border-2 border-emerald-200 shadow-lg rounded-xl p-6 hover:shadow-xl transition-all duration-300 hover:scale-[1.02] group">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                        <span className={`px-3 py-1 rounded-full text-sm font-bold border ${wishlist.is_public
                    ...
                    <div className="flex gap-2 mt-4 flex-wrap">
                        {showUsersInterested && (
                            <button
                                className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold shadow-lg hover:bg-emerald-700 transition-colors"
                                onClick={handleShowInterested}
                            >
                                Users Interested
                            </button>
                        )}
                        ...
                        {!isOwner && (
                            <button
                                className="px-4 py-2 rounded-xl bg-blue-600 text-white font-bold shadow-lg hover:bg-blue-700 transition-colors"
            ...
            {showInterested && (
                <UsersInterestedModal wishlists={interestedWishlists} onClose={() => setShowInterested(false)} />
            )}
            {showAddModal && (
                <AddToWishlistModal onClose={() => setShowAddModal(false)} />
            )}
            ...
            />
            {showShareModal && (
                <ShareWishlistModal link={shareUrl} onClose={() => setShowShareModal(false)} />
            )}
        </div>
    )
}
